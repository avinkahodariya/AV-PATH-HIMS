...
import { CommonConstant, LocationService, ErrorConstant, PaginationType } from "utility";

export const GetLocationList = (paginationType = PaginationType.default) => {
...
export const GetLocationDetail = (Id) => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const result = await LocationService.byId({ Id });
            setData(result.data);
        } catch {
            setError(ErrorConstant.default);
        } finally {
