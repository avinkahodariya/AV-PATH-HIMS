...
import { CommonConstant, CompanyService, ErrorConstant, PaginationType } from "utility";
import { useAuth } from "context";

export const GetCompanyList = (paginationType = PaginationType.default) => {
    const { user } = useAuth()
...
export const GetCompanyDetail = (Id) => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const result = await CompanyService.byId({ Id });
            setData(result.data);
        } catch {
            setError(ErrorConstant.default);
        } finally {
