...
import { RegionService, FormModes, NotificationStatus } from "utility";
import { GetRegionDetail, reactToaster } from "hooks";
import { useEffect } from "react";
import dayjs from "dayjs";
import { GetRegionList } from "hooks";
...
export function Region() {
    ...
    const { data, loading, refresh, filter, totalCount, pageChanged, filterChanged } = GetRegionList()
...
const AddEditregionModel = ({ mode, open, onSubmit, data }) => {
    const { data: detail } = GetRegionDetail(data?.id)
    ...
    const submit = async (formData) => {
        try {
            ...
            const result = await RegionService.add(formData)
    ...
    return (
        <PopUpModal open={open}>
            <ModalHeader mode={mode} title="Region" />
            <div className=" max-h-[65vh] overflow-y-auto p-4 w-[30vw]  ">
                ...
                />
                    <FormCheckBox
                        ...
                        defaultValue={false}
                        className={`${errors.Region && "mb-6"} col-span-1`}
                    />
                </div>
            </div>
            <SubmitCancelButtons
