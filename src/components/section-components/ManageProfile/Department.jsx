...
import { DepartmentService, FormModes, NotificationStatus } from "utility";
import { GetDepartmentDetail, reactToaster } from "hooks";
import { useEffect } from "react";
import dayjs from "dayjs";
import { GetDepartmentList } from "hooks";
...
export function Department() {
    ...
    const { data, loading, refresh, filter, totalCount, pageChanged } = GetDepartmentList()
    ...
    const columns = [
        ...
        { header: "departmentName", field: "departmentName" },
...
const AddEditDepartMentModel = ({ mode, open, onSubmit, data }) => {
    const { data: detail } = GetDepartmentDetail(data.id)
    ...
    const submit = async (formData) => {
        try {
            ...
            const result = await DepartmentService.add(formData)
    ...
    return (
        <PopUpModal open={open}>
            <ModalHeader mode={mode} title="Department" />
            <div className=" max-h-[65vh] overflow-y-auto p-4 w-[30vw]  ">
                <div className="grid grid-cols-2 gap-2  gap-x-4 gap-y-3 md:grid-cols-2 max-h-[100vh] overflow-y-auto  ">
                    ...
                    <FormCheckBox
                        ...
                        defaultValue={false}
                        className={`${errors.Department && "mb-6"} col-span-1`}
                    />
                </div>
            </div>
            <SubmitCancelButtons
