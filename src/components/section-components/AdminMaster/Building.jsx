...
import { BuildingService, FormModes, NotificationStatus } from "utility";
import { GetBuildingDetail, reactToaster } from "hooks";
import { useEffect } from "react";
import dayjs from "dayjs";
import { GetBuildingList } from "hooks";
...
export function Building() {
    ...
    const { data, loading, refresh, filter, totalCount, pageChanged, filterChanged } = GetBuildingList()
...
const AddEditBuildingModel = ({ mode, open, onSubmit, data }) => {
    const { data: detail } = GetBuildingDetail(data.id)
    ...
    const submit = async (formData) => {
        try {
            ...
            const result = await BuildingService.add(formData)
    ...
    return (
        <PopUpModal open={open}>
            <ModalHeader mode={mode} title="Building " />
            <div className=" max-h-[65vh] overflow-y-auto p-4 w-[30vw]  ">
                ...
                />
                    <FormCheckBox
                        ...
                        defaultValue={false}
                        className={`${errors.Building && "mb-6"} col-span-1`}
                    />
                </div>
            </div>
            <SubmitCancelButtons
